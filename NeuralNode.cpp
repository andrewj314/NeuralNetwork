...
NeuralNode::NeuralNode(int layerIndex, string function){
	//setBias(false);
...
void NeuralNode::addDownstreamConnection(Connection *connection){
	m_downstreamConnections.push_back(connection);
}

void NeuralNode::addUpstreamConnection(Connection *connection){
	m_upstreamConnections.push_back(connection);
}

void NeuralNode::addDownstreamConnections(vector<Connection*> connections){
...
void NeuralNode::addUpstreamConnections(vector<Connection*> connections){
	vector<Connection*>::iterator connectionIter;
	for(connectionIter = connections.begin(); connectionIter != connections.end(); connectionIter++){
...
void NeuralNode::backProp(){
	if(isBiasNode() || isInputNode()) getDelta();
		...
		for(vector<Connection*>::iterator connectIter = m_upstreamConnections.begin(); connectIter != m_upstreamConnections.end(); ++connectIter){
			(*connectIter)->getInputNode()->backProp();
		}
	}
}
...
bool NeuralNode::isBiasNode(){
	return m_isBiasNode;
}

bool NeuralNode::isInputNode(){
	return (isBiasNode() == false && getNUpstreamConnections() == 0);
}

bool NeuralNode::isOutputNode(){
	return (isBiasNode() == false && getNDownstreamConnections() == 0);
}

vector<Connection*> NeuralNode::getDownstreamConnections(){
	return m_downstreamConnections;
}

vector<Connection*> NeuralNode::getUpstreamConnections(){
...
double NeuralNode::sigmoidDerivative(double inputSum){
	return sigmoid(inputSum) * (1.0 - sigmoid(inputSum));
}
